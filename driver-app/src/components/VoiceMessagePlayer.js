...
function durationFromMeta(mediaMeta) {
...
export function VoiceMessagePlayer({ uri, mediaMeta, isMine }) {
  ...
  const accent = isMine ? '#FFFFFF' : '#3FC060';
  const sub = isMine ? 'rgba(255,255,255,0.9)' : '#64748B';
  const trackBg = isMine ? 'rgba(255,255,255,0.35)' : '#E2E8F0';
  const fillBg = isMine ? '#FFFFFF' : '#3FC060';
  ...
  const timeLabel = useMemo(() => {
    if (totalMs > 0) {
      return `${formatMs(positionMs)} / ${formatMs(totalMs)}`;
    }
  ...
  return (
    <View style={styles.wrap}>
      ...
      <Text style={[styles.hint, { color: sub }]}>
        Appuyez sur la barre pour avancer · −10s / +10s
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  skipBtn: { alignItems: 'center', paddingHorizontal: 4, paddingVertical: 2 },
  skipLabel: { fontSize: 9, fontWeight: '700', marginTop: -2 },
  time: { fontSize: 12, fontWeight: '600', marginLeft: 4, fontVariant: ['tabular-nums'] },
  track: {
    height: 6,
    borderRadius: 3,
    overflow: 'visible',
