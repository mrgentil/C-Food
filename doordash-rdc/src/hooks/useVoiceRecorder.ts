...
type Phase = 'idle' | 'preparing' | 'recording';
...
export function useVoiceRecorder(onRecorded: (uri: string) => Promise<void>) {
  ...
  useEffect(
    () => () => {
      ...
      if (rec) rec.stopAndUnloadAsync().catch(() => undefined);
    },
    []
  );
  ...
  const stopAndSend = useCallback(async () => {
    ...
    try {
      ...
      setPhase('idle');
      setDurationMs(0);
      const msg = e instanceof Error ? e.message : 'Envoi vocal impossible';
