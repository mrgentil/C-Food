...
import { PET_ITEMS } from '../data/mockData';
...
type RouteParams = RouteProp<RootStackParamList, 'PetStoreDetail'>;
...
const SUBCATEGORIES = ['Tous', 'Nourriture', 'Accessoires', 'Hygiène', 'Jouets', 'Vétérinaire'];

export default function PetStoreDetailScreen() {
  ...
  const items = PET_ITEMS[store.id] || [];
  ...
  const getPetIcon = (petType: string) => {
    return petType === 'Chien' ? 'paw' : 'cat';
  };

  const getPetColor = (petType: string) => {
    return petType === 'Chien' ? '#8B4513' : '#6B7280';
  };
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.heroContainer}>
        ...
        <View style={styles.deliveryBadge}>
          <Ionicons name="time-outline" size={14} color={PET_COLORS.primary} />
      ...
      <View style={styles.infoCard}>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={styles.minOrderBadge}>
            <Ionicons name="paw" size={14} color={PET_COLORS.primary} />
            ...
            <View key={item.id} style={styles.productCard}>
              <View style={styles.productInfo}>
                <View style={styles.productHeader}>
                  ...
                  <View style={[styles.petBadge, { backgroundColor: getPetColor(item.petType) + '20' }]}>
                    <Ionicons name={getPetIcon(item.petType) as any} size={12} color={getPetColor(item.petType)} />
                    <Text style={[styles.petBadgeText, { color: getPetColor(item.petType) }]}>{item.petType}</Text>
                  </View>
                </View>
              ...
              >
                <Ionicons name="add" size={24} color={PET_COLORS.primary} />
              </TouchableOpacity>
            </View>
          ))
        )}
...
const styles = StyleSheet.create({
  ...
  tabActive: {
    borderBottomColor: PET_COLORS.primary,
  },
  tabText: {
    fontSize: FONT_SIZES.md,
    fontWeight: '500',
